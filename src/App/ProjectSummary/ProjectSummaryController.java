...
public class ProjectSummaryController implements Initializable {
    ...
    public void initializeProjects(String userRole){
        ...
        if(userRole.matches("ADMIN_AUTH")) {
            ...
            sql = "SELECT distinct PROJECT_INFO.id, project_name, start_date, end_date, estimated_time FROM PROJECT_INFO INNER JOIN PROJECT_TASK ON PROJECT_INFO.id = PROJECT_TASK.id AND assigned="+ getEmployeeId();
